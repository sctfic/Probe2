...
class ConfigManager {
    ...
    saveConfig(stationId, config) {
        ...
        try {
            ...
            console.error(`${V.error} Erreur de sauvegarde pour ${stationId}:`, error, configPath);
            return false;
        }
    }
    autoSaveConfig(config) {
        const configPath = path.join(CONFIG_DIR, `${config.id}.json`);
        try {
            fs.writeFileSync(configPath, JSON.stringify(config, null, 4));
            // console.log(`${V.database} Configuration pour ${config.id} sauvegardée avec succès. ${V.Check}`);
            return true;
        } catch (error) {
            console.error(`${V.error} Erreur de sauvegarde pour ${config.id}:`, error, configPath);
            return false;
        }
    }
