...
async function sendCommand(req, stationConfig, command, timeout = 2000, answerFormat = "") {
    ...
    while (attempts < maxAttempts) {
        attempts++;
        console.log(`${V.send} Sending to ${stationConfig.id} [${stationConfig.host}:${stationConfig.port}] (${attempts}/${maxAttempts}): '`, commandText, command, `', AnswerFormat: ${answerFormat}`);
